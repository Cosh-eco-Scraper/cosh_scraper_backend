...
describe("OpeningHoursService", () => {
    ...
    describe("updateOpeningHours", () => {
        it("should update opening hours successfully (happy case)", async () => {
            ...
            const updateOpeningHoursStub = sinon.stub(openingHoursRespository, "updateOpeningHours").resolves();

            // Act
            await OpeningHoursService.updateOpeningHours(openingHoursId, day, startTime, endTime, store_id);

            // Assert
            expect(updateOpeningHoursStub.calledOnceWith(openingHoursId, day, startTime, endTime, store_id)).to.be.true;
        });
