...
describe('LocationService', () => {
  ...
  describe('updateLocation', () => {
    it('should update a location successfully (happy case)', async () => {
      ...
      // Assert
      expect(
        updateLocationStub.calledOnceWith(locationId, street, number, postal_code, city, country),
      ).to.be.true;
    });
    ...
    it('should throw an error if the repository fails (unhappy case)', async () => {
      ...
      const updateLocationStub = sinon
        .stub(LocationRepository, 'updateLocation')
